...
Used the same program as 115, just add a loop to test when CalculateOrderingWays() exceeds 1M.
...
long unsigned int CalculateOrderingWays(long unsigned int m, long unsigned int n) {
    ...
    for (long unsigned int k = m; k <= n-1; k++) {
        // Iterate through possible numbers of red items
        long unsigned int g = n - k; // Number of grey itemsint argc, char **argv
        ...
        for (long unsigned int m_R = 1; m_R <= k/m; m_R++) {
            // Number of ways to partition 'k' red items into 'm_R' blocks, each >= 3
            long unsigned int num_red_block_compositions = CountCompositions(k, m_R, m);
...
int main () {
    long unsigned int n, m, r;

    m = 50;
    n = 100;

    while ((r = CalculateOrderingWays(m, n)) < 1000000)
        n++;
